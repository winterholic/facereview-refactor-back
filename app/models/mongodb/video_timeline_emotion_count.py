...
class VideoTimelineEmotionCountRepository:
    ...
    def increment_emotion(self, video_id: str, youtube_running_time: float, emotion: str):
        ...
        logger.debug(f"타임라인 집계 시작: video_id={video_id}, original_time={youtube_running_time}, time_key={time_key}, emotion={emotion}")
        ...
        logger.debug(f"타임라인 감정 집계 완료: video_id={video_id}, time={time_key}, emotion={emotion}")
    ...
    def compensate_upsert(self, compensation_data: Dict[str, any]):
        ...
        if was_insert:
            ...
            self.collection.delete_one({'video_id': video_id})
            logger.info(f"영상 타임라인 감정 집계 삭제: {video_id}")
        else:
            ...
            if previous_data:
                ...
                logger.info(f"영상 타임라인 감정 집계 복원: {video_id}")
    ...
    def compensate_delete(self, compensation_data: Dict[str, any]):
        ...
        if deleted_data:
            ...
            logger.info(f"삭제된 영상 타임라인 감정 집계 복원: {deleted_data['video_id']}")
